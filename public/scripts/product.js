Vue.component("product", {
  ...
  methods: {
    sortByNameOnChange(ev) {
      this.$refs["product-list"].setSortType(ev);
      this.$refs["product-list"].getProducts();
    },

    showOnchange(ev) {
      this.$refs["product-list"].setVisibleCount(+ev);
      this.$refs["product-list"].getProducts();
    }
  },

  template: `<div class="product-box">
  <product-filter :sortType='sortType' v-on:sortbyname-onchange='sortByNameOnChange' v-on:show-onchange='showOnchange'></product-filter>  
    <product-list ref='product-list' :visCount='visibleCount' :typeOfSort='sortType' :extendedButtonSet=true></product-list>
